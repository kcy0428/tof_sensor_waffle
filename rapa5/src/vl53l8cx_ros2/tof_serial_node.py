...
class TofSerialNode(Node):
    ...
    def _open_serial(self, port: str, baudrate: int):
        candidates = [port, '/dev/ttyACM0', '/dev/ttyUSB0', '/dev/ttyACM1']
        seen: list[str] = []
        for p in candidates:
            if p in seen:
                continue
            ...
            try:
                ...
                ser.xonxoff  = False
                ser.open()
                self.get_logger().info(f'시리얼 연결 성공: {p} @ {baudrate}bps')
                return ser
            except serial.SerialException as e:
                self.get_logger().warning(f'{p} 연결 실패: {e}')
        return None
